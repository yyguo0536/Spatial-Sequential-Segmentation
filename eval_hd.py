# -*- coding: utf-8 -*-
"""
Created on Tue Aug 21 14:53:09 2018
...
class Surface(object):
    ...
    Bornik, A.; Cashman, P.; Ying Chi; Cordova, A.; Dawant, B.M.; Fidrich, M.; Furst, J.D.;
    Furukawa, D.; Grenacher, L.; Hornegger, J.; Kainmuller, D.; Kitney, R.I.; Kobatake, H.;
    Lamecker, H.; Lange, T.; Jeongjin Lee; Lennon, B.; Rui Li; Senhu Li; Meinzer, H.-P.;
    Nemeth, G.; Raicu, D.S.; Rau, A.-M.; van Rikxoort, E.M.; Rousson, M.; Rusko, L.;
    Saddi, K.A.; Schmidt, G.; Seghers, D.; Shimizu, A.; Slagmolen, P.; Sorantin, E.;
    Soza, G.; Susomboon, R.; Waite, J.M.; Wimmer, A.; Wolf, I.; , "Comparison and 
    Evaluation of Methods for Liver Segmentation From CT Datasets," Medical Imaging, 
    IEEE Transactions on , vol.28, no.8, pp.1251-1265, Aug. 2009
    ...
    def __init__(self, mask, reference, physical_voxel_spacing = [1,1,1], 
        ...
        mask_edge_points = mask_edge_points.astype(np.float64)
        mask_edge_points *= physical_voxel_spacing
        reference_edge_points += scipy.array(reference_offset)
        reference_edge_points = reference_edge_points.astype(np.float64)
    ...
    def hausdorff(self):
        dists = []
        l = len(self.__reference_edge_points)
        for i in range(l):
            dist_min = 1000.0
            radius = 0
            b_mod = np.empty(shape=(0, 0, 0))
            while b_mod.shape[0] == 0:
                b_mod = self.bbox(self.__mask_edge_points, self.__reference_edge_points[i], radius)
                radius += 1
        ...
        # increasing the size of the cube
            b_mod = self.bbox(self.__mask_edge_points, self.__reference_edge_points[i], radius * math.sqrt(3))

            for j in range(len(b_mod)):
                dist = np.linalg.norm(self.__reference_edge_points[i] - b_mod[j])
                if dist_min > dist:
                    dist_min = dist
