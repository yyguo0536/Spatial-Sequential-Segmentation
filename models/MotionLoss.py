...
class angleLoss(torch.nn.Module):
    ...
    def __init__(self, channels=1):
        ...
        self.conv3d=torch.nn.Conv3d(\
            in_channels=1, out_channels=1, kernel_size=(3,3,3), stride=1, groups=channels,\
            padding = (1,1,1),bias=False)
        x_cord = torch.Tensor([1,1,1])
        x_cord = x_cord.repeat(3).view(3,3)
        x_cord = x_cord.expand(3,3,3)
        x_cord[1,1,1] = -8.0
        x_cord = x_cord.expand(channels,3,3,3)
...
class gradientLoss(torch.nn.Module):
    ...
    def forward(self, feild):
        ...
        distance = torch.mean(x_dx) + torch.mean(x_dy) + torch.mean(x_dz) +\
                    ...
                    torch.mean(z_dz) + torch.mean(z_dx) + torch.mean(z_dy)
...
class fieldLoss(torch.nn.Module):
    ...
    def forward(self, feild):
        ...
        #gradient_d = torch.mean(self.conv3d(dz)) + torch.mean(self.conv3d(dy)) + torch.mean(self.conv3d(dx))

        return distance#(torch.abs(gradient_d/9.0))
...
class similarLoss(torch.nn.Module):
    ...
    def __init__(self, channels=1):
        ...
        self.conv3d=torch.nn.Conv3d(\
            in_channels=1, out_channels=1, kernel_size=(9,9,9), stride=1, groups=channels,\
            padding = (4,4,4),bias=False)
        x_cord = torch.Tensor([1,1,1,1,1,1,1,1,1])
        x_cord = x_cord.repeat(9).view(9,9)
        x_cord = x_cord.expand(9,9,9)
        x_cord = x_cord.expand(channels,9,9,9)
        x_cord = torch.unsqueeze(x_cord,0)
